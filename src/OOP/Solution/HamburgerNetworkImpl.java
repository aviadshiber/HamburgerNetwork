...
public class HamburgerNetworkImpl implements HamburgerNetwork {
    ...
    public Collection<Restaurant> favoritesByRating(HungryStudent s) throws StudentNotInSystemException {
        ...
        s.getFriends().stream().sorted(Comparable::compareTo).forEachOrdered(friend -> result.addAll(friend.favoritesByDist(Integer.MIN_VALUE)));
        return result;
    }
    ...
    public Collection<Restaurant> favoritesByDist(HungryStudent s) throws StudentNotInSystemException {
        ...
        validateStudentPresence(studentID);
        Set<Restaurant> result = new TreeSet<>();
        s.getFriends().stream().sorted(Comparable::compareTo).forEachOrdered(friend -> result.addAll(friend.favoritesByRating(Integer.MAX_VALUE)));
        return result;
    }
